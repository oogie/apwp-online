...
export const comp = {
    ...
    view: function (vnode) {
        ...
        return (
            <div class='comp rotgraph'>
                <div id='graphWrapper'>
                    <div id='rotgraphContent'>Loading graph..</div>
                </div>
                ...
                <Positionaldialog fillcontainer={true} isOpen={vnode.state.isOpen.export} onClose={() => {vnode.state.isOpen.export = false}}>
                    <div class='menu'>
                        ...
                        <a onclick={() => { vnode.state.graph.exportChartLocal({ type: 'application/pdf', filename: 'Relative rotation APWP-online.org' }); vnode.state.isOpen.export = false }} href='javascript:'>
                            <i class="fa-solid fa-download fa-fw"></i> Download PDF
                        </a>

                        <div class='spacer' />
                        <a onclick={() => { vnode.state.graph.exportChartLocal({ type: 'image/png', filename: 'Relative rotation APWP-online.org' }); vnode.state.isOpen.export = false }} href='javascript:'>
                            <i class="fa-solid fa-download fa-fw"></i> Download PNG
                        </a>
                        <a onclick={() => { vnode.state.graph.exportChartLocal({ type: 'image/jpeg', filename: 'Relative rotation APWP-online.org' }); vnode.state.isOpen.export = false }} href='javascript:'>
                            <i class="fa-solid fa-download fa-fw"></i> Download JPG
                        </a>
                        <a onclick={() => { vnode.state.graph.exportChartLocal({ type: 'image/svg+xml', filename: 'Relative rotation APWP-online.org' }); vnode.state.isOpen.export = false }} href='javascript:'>
...
function initGraph(vnode) {
    const graph = Highcharts.chart('rotgraphContent', {
        exporting: {
            buttons: {
                contextButton: {
                    enabled: false
            ...
            formatter: function() {
                ...
                return `
                    <div>
                        ...
                        <div>Diff: <b>${context.R.R.toFixed(1)}°</b> &pm; <small>${context.R.delta_R.toFixed(1)}°</small></div>
                    </div>
                `
            }
        },
...
function updateSeries(vnode) {
    ...
    vnode.state.graph.addSeries({
        ...
        data: vnode.attrs.rpd.map((p) => {
            return [
                { x: p.input_pole.max_age === 0 ? null : p.input_pole.max_age, y: p.R.R, marker: { symbol: 'vline', lineWidth: 0.5, lineColor: null }},
                { x: p.input_pole.max_age === 0 ? null : p.input_pole.min_age, y: p.R.R, marker: { symbol: 'vline', lineWidth: 0.5, lineColor: null }},
                { x: null, y: null },
                { x: p.input_pole.age, y: p.R.R + p.R.delta_R, marker: { symbol: 'hline', lineWidth: 0.5, lineColor: null }},
                { x: p.input_pole.age, y: p.R.R - p.R.delta_R, marker: { symbol: 'hline', lineWidth: 0.5, lineColor: null }},
                { x: null, y: null }
            ]
        }).flat()
