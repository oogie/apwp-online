            ...
            formatter: function() {
                ...
                return `
                    <div>
                        ...
                        <div>Diff: <b>${context.R.toFixed(1)}°</b> &pm; <small>${context.delta_R.toFixed(1)}°</small></div>
                    </div>
                `
            }
        },
...
function updateSeries(vnode) {
    ...
    vnode.state.graph.addSeries({
        ...
        data: vnode.attrs.rpd.map((p) => {
            return [
                {x: p.max_age === 0 ? null : p.max_age, y: p.R, marker: { symbol: 'vline', lineWidth: 0.5, lineColor: null }},
                {x: p.max_age === 0 ? null : p.min_age, y: p.R, marker: { symbol: 'vline', lineWidth: 0.5, lineColor: null }},
                {x: null, y: null},
                {x: p.age, y: p.R + p.delta_R, marker: { symbol: 'hline', lineWidth: 0.5, lineColor: null }},
                {x: p.age, y: p.R - p.delta_R, marker: { symbol: 'hline', lineWidth: 0.5, lineColor: null }},
                {x: null, y: null},
            ]
        }).flat()
    ...
    vnode.state.graph.addSeries({
        ...
        data: vnode.attrs.rpd.map((p) => {
            return {
                marker: {
                    fillColor: p.R_sig ? undefined : "white",
                    lineWidth: p.R_sig ? undefined : 1,
                    lineColor: "#0652DD",
                    symbol: "circle"
                },
                x: p.age,
